...
from .constants import MUSE_SCAN_TIMEOUT, AUTO_DISCONNECT_DELAY, \
    ...
    MUSE_NB_GYRO_CHANNELS, MUSE_SAMPLING_GYRO_RATE, LSL_GYRO_CHUNK, LSL_SCAN_TIMEOUT
from .viewer_v1 import LSLViewer
...
def stream(
    ...
    if backend != 'bluemuse':
        ...
        if not eeg_disabled:
            ...
            # Data is thrown into a Stream outlet (we need to catch this somewhere :D
        ...
        if (didConnect):
            ...
            # Prints the names of the data streams based on the booleans given in the main.py script
            print("Streaming%s%s%s%s..." %
                  (eeg_string, ppg_string, acc_string, gyro_string))

            # Finds all streams that are of type 'EEG', reference: beginning of this method, where we create the Stream Outlet
            streams = resolve_byprop('type', 'EEG', timeout=LSL_SCAN_TIMEOUT)
            stream_inlet = StreamInlet(streams[0], max_chunklen=LSL_EEG_CHUNK)

            PPG_streams = resolve_byprop('type', 'PPG', timeout=LSL_SCAN_TIMEOUT)
            PPG_stream_inlet = StreamInlet(PPG_streams[0], max_chunklen=LSL_PPG_CHUNK)

            ACC_streams = resolve_byprop('type', 'ACC', timeout=LSL_SCAN_TIMEOUT)
            ACC_stream_inlet = StreamInlet(ACC_streams[0], max_chunklen=LSL_ACC_CHUNK)

            GYRO_streams = resolve_byprop('type', 'GYRO', timeout=LSL_SCAN_TIMEOUT)
            GYRO_stream_inlet = StreamInlet(GYRO_streams[0], max_chunklen=LSL_GYRO_CHUNK)

            # tries to fetch the stream and start using the callback method on the stream
            while time() - muse.last_timestamp < timeout:
                try:
                    backends.sleep(1)

                    eeg_data, timestamps_eeg = stream_inlet.pull_chunk(timeout=1.0, max_samples=LSL_EEG_CHUNK)
                    ppg_data, timestamps_ppg = PPG_stream_inlet.pull_chunk(max_samples=LSL_PPG_CHUNK)
                    acc_data, timestamps_acc = ACC_stream_inlet.pull_chunk(max_samples=LSL_ACC_CHUNK)
                    gyro_data, timestamps_gyro = GYRO_stream_inlet.pull_chunk(max_samples=LSL_GYRO_CHUNK)

                    data_handler.handle_eeg_data(data=eeg_data, timestamps=timestamps_eeg)
                    data_handler.handle_ppg_data(ppg_data=ppg_data, timestamps=timestamps_ppg)
                    data_handler.handle_acc_data(acc_data=acc_data, timestamps=timestamps_acc)
                    data_handler.handle_gyro_data(gyro_data=gyro_data, timestamps=timestamps_gyro)
    ...
    # For bluemuse backend, we don't need to create LSL streams directly, since these are handled in BlueMuse itself.
        ...
        subprocess.call('start bluemuse://setting?key=eeg_enabled!value={}'.format('false' if eeg_disabled else 'true'),
                        shell=True)
        subprocess.call('start bluemuse://setting?key=ppg_enabled!value={}'.format('true' if ppg_enabled else 'false'),
                        shell=True)
        subprocess.call(
            'start bluemuse://setting?key=accelerometer_enabled!value={}'.format('true' if acc_enabled else 'false'),
            shell=True)
        subprocess.call(
            'start bluemuse://setting?key=gyroscope_enabled!value={}'.format('true' if gyro_enabled else 'false'),
            shell=True)
