...
from muselsl.constants import VIEW_BUFFER, VIEW_SUBSAMPLE, LSL_SCAN_TIMEOUT, LSL_EEG_CHUNK
import matplotlib.pyplot


def view(timestamps, streams, inlet, data, window, scale, refresh, figure, backend, version=1):
    ...
    lslv = LSLViewer(stream= streams[0], inlet=inlet, fig=fig, axes=axes, window=window, scale=scale)
    ...
    help_str = """
               ...
               """
    print(help_str)
    lslv.start(stream=streams, timestamp=timestamps, data= data)
    matplotlib.pyplot.show()


class LSLViewer():
    def __init__(self, stream, inlet, fig, axes, window, scale, dejitter=True):
        """Init"""
        self.stream = stream
        self.window = window
        self.scale = scale
    ...
    def update_plot(self, timestamps, data):
        ...
        try:
            while self.started:
                print('jetzt kommt pull chunk')
                samples, timestamps= data, timestamps
    ...
    def start(self, stream, timestamp, data):
        self.started = True
        self.thread = Thread(target=self.update_plot(timestamp, data= data))
